...
public class FileUtil {
    ...
    public static String toLinuxPath(File file) {
        return toLinuxPath(file.getAbsoluteFile());
    }

    public static File createPath(File baseDir, Comparable... subs) {
        StringBuilder relative = new StringBuilder();
        for (Comparable sub : subs) {
            relative.append(sub).append(File.separator);
        }
        File path = new File(baseDir, relative.toString());
        path.mkdirs();
        return path;
    }

    public static void createParent(File file) throws IOException {
        Files.createParentDirs(file);
    }

    public static String toLinuxPath(String path) {
        if (!File.pathSeparator.equals("/") && path.contains(File.pathSeparator)) {
